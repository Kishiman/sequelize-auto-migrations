...
const reverseModels = function (sequelize, models) {
  ...
  for (let model in models) {
    let attributes = models[model].rawAttributes || models[model].tableAttributes;
        ...
        return new Promise(function(resolve, reject) {
          async function next() {
            if (index < migrationCommands.length) {
                ...
                if (command.fn == "changeColumn" && command.params[2] && .allowNull != undefined) {
                    const { allowNull } = command.params[2]
                    console.log("[#" + index + "] allowNull: " + command.params[0] + "." + command.params[1]);
                    let type = command.params[2]["type"]
                    if (!("toSql" in type))
                        type = type()
                    console.log(type.toSql());
                    await queryInterface.sequelize.query(\`ALTER TABLE \\\`\${command.params[0]}\\\` CHANGE \\\`\${command.params[1]}\\\` \\\`\${command.params[1]}\\\` \${type.toSql()} \${allowNull == false ? 'NOT' : ''} NULL\`)
                }
                index++;
